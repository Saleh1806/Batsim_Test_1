...
#ifdef USE_SLEEPER
#include "sleeper.hpp"  // Si Sleeper est une bibliothèque externe, on l'inclut ici.
#endif
...
// Déclaration conditionnelle de l'objet Sleeper si nécessaire
#ifdef USE_SLEEPER
...
uint8_t batsim_edc_init(const uint8_t * data, uint32_t size, uint32_t flags) {
    ...
    #ifdef USE_SLEEPER
    sleeper = new Sleeper(/* paramètres nécessaires */);
    #endif
...
uint8_t batsim_edc_deinit() {
    ...
    // Suppression conditionnelle de l'objet Sleeper si nécessaire
    #ifdef USE_SLEEPER
    delete sleeper;
    sleeper = nullptr;
    #endif
...
uint8_t batsim_edc_take_decisions(
    ...
    for (unsigned int i = 0; i < nb_events; ++i) {
        ...
        case fb::Event_SimulationBeginsEvent: {
            ...
            #ifdef USE_SLEEPER
            sleeper->on_simulation_start(parsed->now(), event);
            #endif
        } break;
    ...
    #ifdef USE_SLEEPER
    if (sleeper != nullptr) {
        sleeper->make_decisions(parsed->now(), nullptr, nullptr);
    }
    #endif
