...
{
    ENFORCE(_placement == nullptr, "placement has already been set");
...
{
    ENFORCE(_profile_overrides.find(profile_id) == _profile_overrides.end(), "placement has already been set for profile '%s'", profile_id.c_str());
    profile_placement = new ProfilePlacement();
...
{
    ENFORCE(_storage_overrides.find(storage_name) == _storage_overrides.end(), "placement has already been set for storage '%s'", storage_name.c_str());
...
ExecuteJobOptions::Placement * ExecuteJobOptions::Placement::make_predefined(fb::ExecutorPlacementStrategy predefined_strategy)
{
    ...
    placement._placement_type = PlacementType::PREDEFINED_STRATEGY;
...
{
    auto placement = new Placement();
    placement._placement_type = PlacementType::CUSTOM;
